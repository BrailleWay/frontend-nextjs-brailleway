...
export async function registerPatient(data) {
  try {
    ...
    console.log(
      `[SUCESSO] Paciente cadastrado: ${paciente.email} (ID: ${paciente.id})`
    );
    return {
      success: true,
...
export async function registerMedic(data) {
  try {
    ...
    const existingEmail = await prisma.medico.findUnique({
      where: { email: email },
    });
    if (existingEmail) {
      return {
    ...
    if (!data.crm || !data.crm.trim()) {
      return { success: false, message: "CRM é obrigatório." };
    }

    const crm = data.crm.trim().toUpperCase();
    const crmRegex = /^[0-9]{4,}-[A-Z]{2}$/;
    if (!crmRegex.test(crm)) {
      return {
        success: false,
        message: "CRM inválido. Formato esperado: 0000-XX.",
      };
    }
    ...
    if (!data.specialtyId || isNaN(parseInt(data.specialtyId))) {
    ...
    if (data.disponibilidades) {
      ...
      for (const disponibilidade of disponibilidades) {
        if (
          ...
          const fakeDate = "1970-01-01";
          await prisma.disponibilidadeMedico.create({
            data: {
              medicoId: medico.id,
              diaSemana: parseInt(disponibilidade.diaSemana, 10),
              horaInicio: new Date(`${fakeDate}T${disponibilidade.horaInicio}`),
              horaFim: new Date(`${fakeDate}T${disponibilidade.horaFim}`),
              disponivel: true,
            },
          });
      ...
      console.log(
        `✅ Criadas ${disponibilidadesCriadas} disponibilidades para o médico ${medico.nome}`
      );

      if (disponibilidadesCriadas === 0) {
        console.warn(
          `⚠️ Médico ${medico.nome} cadastrado sem disponibilidades configuradas`
        );
      }
    } else {
    ...
    return {
      ...
      message:
        ...
        (data.disponibilidades
          ? " Lembre-se de configurar os horários de disponibilidade."
          : ""),
    };
  } catch (error) {
...
export async function verificarDisponibilidade(criteria) {
  ...
  try {
    ...
    if (criteria.nome_medico) {
      ...
      console.log(
        `[MATCH] Melhor correspondência: '${todosMedicos[bestMatchIndex]?.nome}' (score ${bestMatch.rating})`
      );
      ...
      if (bestMatch.rating >= 0.7) {
        ...
        return {
          disponivel: false,
          motivo: "Nenhum médico encontrado com esse nome.",
        };
      }
      ...
      if (medicos.length === 0) {
        return {
          disponivel: false,
          motivo: "Nenhum médico com essa especialidade.",
        };
      }
      console.log(
        `[ESPECIALIDADE] Encontrados ${medicos.length} médicos para '${criteria.especialidade}'`
      );
    }
    ...
    const dataHoraDesejada = parseDateTimeWithTimezone(
      criteria.data,
      criteria.hora
    );
    ...
    console.log(
      `[TIMEZONE DEBUG] Local: ${dataHoraDesejada.toLocaleString("pt-BR", {
        timeZone: TZ,
      })}`
    );

    if (isNaN(dataHoraDesejada.getTime()))
      return { disponivel: false, motivo: "Data ou hora inválida." };
    if (isBefore(dataHoraDesejada, new Date()))
      return {
        disponivel: false,
        motivo: "Não é possível agendar no passado.",
      };
    ...
    for (const medico of medicos) {
      ...
      const horaDesejada = new Date(
        `1970-01-01T${dataHoraDesejada
          .getUTCHours()
          .toString()
          .padStart(2, "0")}:${dataHoraDesejada
          .getUTCMinutes()
          .toString()
          .padStart(2, "0")}:00Z`
      );
      ...
      if (!disponibilidade) {
        console.log(
          `   ↪️  Fora do horário de atendimento (diaSemana=${diaSemana}, hora=${horaDesejada
            .toTimeString()
            .slice(0, 5)})`
        );
        continue;
      ...
      const horaDesejadaMinutos =
        dataHoraDesejada.getHours() * 60 + dataHoraDesejada.getMinutes();
      const horaInicioMinutos =
        disponibilidade.horaInicio.getHours() * 60 +
        disponibilidade.horaInicio.getMinutes();
      const horaFimMinutos =
        disponibilidade.horaFim.getHours() * 60 +
        disponibilidade.horaFim.getMinutes();
      ...
      console.log(
        `   📊 Score: ${scoreTotal} (prox: ${scoreProximidade}, flex: ${scoreFlexibilidade}, ocup: ${scoreOcupacao})`
      );
    }
    ...
    console.log(
      `🏆 Melhor médico escolhido: ${melhorMedico.medico.nome} (score: ${melhorMedico.score})`
    );

    // Se há múltiplos médicos com scores similares, oferecer opções
    const medicosSimilares = medicosDisponiveis.filter(
      (m) =>
        Math.abs(m.score - melhorMedico.score) < 20 &&
        m.medico.id !== melhorMedico.medico.id
    );

    if (medicosSimilares.length > 0 && criteria.especialidade) {
      const opcoes = [
        melhorMedico.medico.nome,
        ...medicosSimilares.map((m) => m.medico.nome),
      ];
      return {
        disponivel: false,
    ...
    return {
      disponivel: true,
      motivo: `Horário encontrado com ${melhorMedico.medico.nome} (${
        melhorMedico.medico.especialidade.nome
      }) em ${dataHoraDesejada.toLocaleString("pt-BR")}.`,
      proximaAcao: {
...
export async function confirmarAgendamento(details) {
  ...
  try {
    ...
    console.log(
      `[TIMEZONE DEBUG] confirmarAgendamento - Input: ${details.dataHora}`
    );
    console.log(
      `[TIMEZONE DEBUG] confirmarAgendamento - Final ISO: ${inicio.toISOString()}`
    );
    console.log(
      `[TIMEZONE DEBUG] confirmarAgendamento - Local: ${inicio.toLocaleString(
        "pt-BR",
        { timeZone: TZ }
      )}`
    );
    ...
    if (isNaN(inicio.getTime()))
      return { success: false, message: "dataHora inválido." };
    ...
    if (conflito)
      return {
        success: false,
        message: "Horário acabou de ser ocupado. Tente outro.",
      };
...
export async function getConsultasUsuario() {
  ...
  try {
    ...
    console.log(
      `[SUCESSO] Foram encontradas ${consultas.length} consultas para o usuário #${session.user.id} (${session.user.role})`
    );
    return consultas;
  } catch (err) {
...
export async function debugAgendamento(criteria) {
  ...
  try {
    ...
    if (criteria.especialidade) {
      ...
      console.log(
        `[DEBUG] Médicos encontrados para '${criteria.especialidade}': ${medicos.length}`
      );
      medicos.forEach((m) => {
        console.log(
          `   - ${m.nome} (${m.especialidade.nome}) - Disponibilidades: ${m.disponibilidades.length}`
        );
      });
    }
    ...
    if (criteria.data && criteria.hora) {
      ...
      console.log(
        `[DEBUG] Data/Hora: ${dataHoraDesejada.toLocaleString("pt-BR")}`
      );
...
export async function login(prevState, formData) {
  ...
  try {
    ...
    // Se o login for bem-sucedido, o Auth.js lançará um erro de redirecionamento.
    // Para garantir a revalidação, a melhor abordagem é redirecionar aqui.
    revalidatePath("/homepage", "layout");
    redirect("/homepage");
  } catch (error) {
    if (error.message.includes("CredentialsSignin")) {
      // Retorna a mensagem de erro para o useFormState
      return "Email ou senha inválidos.";
    }
    // Se não for um erro de login, lança-o para o Next.js tratar
    throw error;
  }
}
